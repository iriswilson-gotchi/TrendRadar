...
def render_ai_analysis_html(result: AIAnalysisResult) -> str:
    ...
    if result.deep_dive:
        content = _format_list_content(result.deep_dive)
        content_html = _escape_html(content).replace("\n", "<br>")
        html_parts.extend([
            '<div class="ai-section">',
            "<h4>值得精读</h4>",
...
def render_ai_analysis_telegram(result: AIAnalysisResult) -> str:
    ...
    if result.core_trends:
        lines.extend(["<b>核心趋势与推演</b>", _escape_html(_format_list_content(result.core_trends)), ""])

    if result.signals:
        lines.extend(["<b>弱信号与异动</b>", _escape_html(_format_list_content(result.signals)), ""])

    if result.deep_dive:
        lines.extend(["<b>值得精读</b>", _escape_html(_format_list_content(result.deep_dive)), ""])
...
def render_ai_analysis_html_rich(result: AIAnalysisResult) -> str:
    ...
    if result.core_trends:
        ...
        ai_html += f"""
                    <div class="ai-block">
                        <div class="ai-block-title">核心趋势与推演</div>
    ...
    if result.signals:
        ...
        ai_html += f"""
                    <div class="ai-block">
                        <div class="ai-block-title">弱信号与异动</div>
                        <div class="ai-block-content">{content_html}</div>
                    </div>"""
    ...
    if result.deep_dive:
        ...
        ai_html += f"""
                    <div class="ai-block">
                        <div class="ai-block-title">值得精读</div>
                        <div class="ai-block-content">{content_html}</div>
                    </div>"""
